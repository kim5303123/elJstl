...
//		요청 처리, 로직 처리 후 데이터 JSP 포워딩
		int iValue = 2025;			//	정수
		float fValue = 3.14f;		//	실수
		String sValue = "Hello EL";	//	문자열
		boolean bValue = true;
...
//		모델 데이터를 요청에 실어서 JSP로 포워딩
		request.setAttribute("iVal",iValue);	//	iVal이란 키워드로 iValue 전달
		...
		request.setAttribute("userVo", vo);		//	userVo에 실어서 vo를 전달
